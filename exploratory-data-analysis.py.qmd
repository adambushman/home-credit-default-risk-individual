...
print(dwell_exp_var[0:4])                               # How much variance is explained by the first 5 components 

```

Okay! The PCA is helping quite a bit here. We've got 84% of the variance explained in the first 5 components. That's great info! For now, however, let's use the original variables in a logistic regression model (I'm curious which, if any, are predictive of `TARGET`).
